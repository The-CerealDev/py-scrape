...
def scrape(url):
    ...
    maindict['(1) Relevant securities owned and/or controlled:'][0] = maindict['(1) Relevant securities owned and/or controlled:'][0].replace(',','')
    
    data = {
        "Company":maindict["(c) Name of offeror/offeree in relation to whose relevant securities this form relates:\n     Use a separate form for each offeror/offeree"][0],
        "Index": "",
        "Filing": "Form 8.3",
        "Position Date": maindict['(e) Date position held/dealing undertaken:\n     For an opening position disclosure, state the latest practicable date prior to the disclosure'][0],
        "Voting Rights": int(maindict['(1) Relevant securities owned and/or controlled:'][0]),
        "%(voting)": float(maindict['(1) Relevant securities owned and/or controlled:'][1][:-1]),
        "Other Instruments": int(maindict['TOTAL'][0])-int(maindict['(1) Relevant securities owned and/or controlled:'][0]),
        ...
        "%(of shares)": float(f"{((int(non_voting.replace(',','')) / int(maindict['(1) Relevant securities owned and/or controlled:'][0])) * 100):.2f}"),
        "%(ISC)": float(f"{((int(non_voting.replace(',','')) / int(maindict['(1) Relevant securities owned and/or controlled:'][0])) * float(maindict['(1) Relevant securities owned and/or controlled:'][1][:-1])):.4f}"),
        "Link": url
    }
    class Company:
